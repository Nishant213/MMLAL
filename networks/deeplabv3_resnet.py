...
class DeepLabv3Plus_RN(nn.Module):
    def __init__(self, backbone, rep_size = 0, num_classes = 11, dilate_scale = 8):
        ...
        if backbone == 'r50_deeplab':
            self.backbone = resnet50(pretrained=True, output_stride=dilate_scale) #default assumption
        elif backbone == 'r50d_deeplab':
            self.backbone = resnet50_d64(pretrained=True, output_stride=dilate_scale) #default assumption
        elif backbone == 'r101_deeplab':
            self.backbone = resnet101(pretrained=True, output_stride=dilate_scale) #default assumption
        elif backbone == 'r101d_deeplab':
            self.backbone = resnet101_d64(pretrained=True, output_stride=dilate_scale) #default assumption
        else:
            raise NotImplementedError("only supporting R50 variants right now")
...
if __name__ == "__main__":
    model = DeepLabv3Plus_RN(backbone='r50d_deeplab', rep_size = 256)
    model.eval()
