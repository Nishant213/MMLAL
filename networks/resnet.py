...
__all__ = ['resnet50', 'resnet50_d64', 'resnet101', 'resnet101_d64']  # model_registry will add each entrypoint fn to this
...
default_cfgs = {
    ...
    'resnet50_d64': _cfg(url='https://www.dropbox.com/s/spx89ruhizycrwa/resnet50-ebb6acbb.pth'), # does not work but needed 
    'resnet101_d64': _cfg(url='https://www.dropbox.com/s/ovswxvo8pauq80r/resnet101-2a57e44d.pth'), # for timm to not throw error
}
...
def resnet50(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3], drop_path_rate = 0.1,  **kwargs)
    return _create_resnet('tv_resnet50', pretrained, **model_args)

# @register_model
# def resnet50x(pretrained=False, **kwargs):
#     """Constructs a ResNet-50 model.
#     """
#     model_args = dict(block=Bottleneck, layers=[3, 4, 6, 3],  drop_path_rate = 0.2, **kwargs)
#     return _create_resnet('tv_resnet50', pretrained, **model_args)
...
def resnet101(pretrained=False, **kwargs):
    ...
    """
    model_args = dict(block=Bottleneck, layers=[3, 4, 23, 3], drop_path_rate = 0.1, **kwargs)
    return _create_resnet('tv_resnet101', pretrained, **model_args)

@register_model
def resnet50_d64(pretrained=False, **kwargs):
    """Constructs a ResNet-50-D model.
    """
    model_args = dict(
        block=Bottleneck, layers=[3, 4, 6, 3], stem_width=64, stem_type='deep', drop_path_rate = 0.1, **kwargs)
    model =  _create_resnet('resnet50_d64', pretrained = False, pretrained_custom_load = True, **model_args)
    if pretrained: 
        model.load_state_dict(torch.load(osp.join('networks', 'weights', 'resnet50-ebb6acbb.pth')))
    return model

@register_model
def resnet101_d64(pretrained=False, **kwargs):
    """Constructs a ResNet-50-D model.
    """
    model_args = dict(
        block=Bottleneck, layers=[3, 4, 23, 3], stem_width=64, stem_type='deep', drop_path_rate = 0.1, **kwargs)
    model = _create_resnet('resnet101_d64', pretrained = False, pretrained_custom_load = True, **model_args)
    if pretrained:
        model.load_state_dict(torch.load(osp.join('networks', 'weights', 'resnet101-2a57e44d.pth')))
    return model

if __name__ == "__main__":
    model = resnet50_d64(output_stride = 8, pretrained=False)
    model.eval()
    ...
    y1, y2, y3, y4 = model.forward_features(input_tensor)
    print("Low level feat shapes: {}".format(y1.shape))
    print("Mid Low level feat shapes: {}".format(y2.shape))
    print("Mid High level feat shapes: {}".format(y3.shape))
    print("High level feat shapes: {}".format(y4.shape))
